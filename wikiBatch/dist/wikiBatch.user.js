...
function switchToProcessingView(itemData) {
    ...
    if (state.previousItem && state.currentIndex > 0) {
        const prevType = state.previousItem.type || 'subject';
    ...
    // Wcode / Infobox area (common to all entity types)
...
function switchToProcessingErrorView(currentItem, errorMsg) {
    ...
    const entityType = currentItem.type || 'subject';
    const TYPE_LABELS = { subject: '条目', character: '角色', person: '人物' };
    const typeLabel = TYPE_LABELS[entityType] || '条目';
...
function switchToUpdateErrorView(errorMsg) {
    ...
    // Determine entity type from current CSV item
