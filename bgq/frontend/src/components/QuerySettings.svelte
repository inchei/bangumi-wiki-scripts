<script>
  import { getFiltersForAPI, queryTarget, lastResult, queryLoading } from '../stores.js';
  ...
  async function handleRun() {
    ...
    try {
      const data = await runQuery(getFiltersForAPI(), cols, get(queryTarget), limit);
      lastResult.set(data);
    } catch (e) {
...
<div class="card">
  ...
  <button class="btn btn-primary btn-block" onclick={handleRun} disabled={loading} style="height:42px;font-size:15px">
    {loading ? '查询中...' : '▶ 执行查询'}
  </button>
</div>
