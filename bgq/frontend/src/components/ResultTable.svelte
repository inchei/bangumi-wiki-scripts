<script>
  import { lastResult, sortState, queryTarget, queryLoading, getFiltersForAPI } from '../stores.js';
  ...
  function sortTable(colIdx) {
    ...
    else { newSort.col = colIdx; newSort.asc = true; }
    sortState.set(newSort);

    const rows = [...res.rows];
    const isNumeric = rows.some((r) => { const p = parseSortVal(r[colIdx]); return !p.empty && !isNaN(p.num); });
    rows.sort((a, b) => {
      const pa = parseSortVal(a[colIdx]), pb = parseSortVal(b[colIdx]);
      if (pa.empty && pb.empty) return 0;
      if (pa.empty) return 1;
      if (pb.empty) return -1;
      const cmp = isNumeric && !isNaN(pa.num) && !isNaN(pb.num) ? pa.num - pb.num : pa.str.localeCompare(pb.str, 'zh');
  ...
  function handleCopyTable() {
    ...
    let text = cols.join('\t') + '\n';
    for (const row of res.rows) text += row.map((v) => v ?? '').join('\t') + '\n';
...
<div class="results-panel">
  ...
  {:else if $lastResult?.rows}
    ...
    <div class="results-toolbar">
      <span class="results-count">共 <b>{res.total_rows}</b> 条结果<span class="time">{res.duration}</span></span>
      <span class="results-actions">
        <button class="btn btn-outline btn-sm" onclick={handleExportCSV}>📥 下载 CSV</button>
        <button class="btn btn-default btn-sm" onclick={handleCopyTable}>📋 复制表格</button>
      </span>
    </div>
    {#if res.rows.length === 0}
      <div class="results-empty"><div class="icon">📭</div><div>没有找到符合条件的条目</div></div>
    {:else}
      <div class="results-table-wrap">
        <table class="results-table">
          <thead>
            <tr>
              {#each res.columns as col, i}
                <th
                  class="sortable"
          ...
          <tbody>
            {#each res.rows as row}
              <tr>
                {#each res.columns as col, i}
                  <td class={cellClass(col)}>{@html cellContent(col, row[i])}</td>
                {/each}
              </tr>
            {/each}
          </tbody>
...
<style>
  ...
  .results-loading { text-align: center; padding: 60px 20px; color: var(--text-secondary); }
  .results-loading .spinner {
    display: inline-block; width: 32px; height: 32px; margin-bottom: 16px;
  ...
  .results-table th {
    background: var(--bg-alt); padding: 10px 14px; text-align: left; font-weight: 600; color: var(--text-secondary);
    font-size: 12px; text-transform: uppercase; letter-spacing: .5px;
    position: sticky; top: 0; z-index: 2; border-bottom: 2px solid var(--border); white-space: nowrap;
  }
  ...
  :global(.results-table .col-id) { font-family: var(--font-mono); font-size: 12px; font-weight: 500; }
  :global(.results-table .col-id a) { color: var(--link); }
  :global(.results-table .col-id a:hover) { color: var(--link-hover); text-decoration: underline; }
  :global(.results-table .col-score) { font-weight: 600; color: var(--accent); }
  :global(.results-table .col-name) { font-weight: 500; }
  :global(.results-table .cell-null) { color: var(--text-placeholder); font-style: italic; }
  .error-card {
    background: var(--error-bg); border: 1px solid var(--error-border); border-radius: var(--radius);
    padding: 16px 20px; color: var(--error-text); font-size: 14px;
  }
  .error-card .error-title { font-weight: 600; margin-bottom: 8px; }
  .error-card pre { background: var(--error-pre-bg); padding: 12px; border-radius: var(--radius-xs); font-size: 12px; font-family: var(--font-mono); overflow-x: auto; max-height: 200px; }
