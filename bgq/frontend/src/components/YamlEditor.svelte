<script>
  import { getFiltersForAPI, applyFiltersFromAPI, queryTarget, logicVersion } from '../stores.js';
  ...
  function handleApply() {
    if (!yamlText.trim()) { alert('YAML 内容为空'); return; }
    error = '';
    try {
  ...
  // Sync on mount
  $effect(() => { syncFromFilters(); });
</script>

<div class="card">
  <div class="card-header"><span class="dot-indicator"></span>YAML 配置</div>
  <textarea class="yaml-editor" bind:value={yamlText} placeholder="在此编辑 YAML 配置..."></textarea>
  {#if error}
    <div class="yaml-error">{error}</div>
  {/if}
  <div class="yaml-actions">
    <button class="btn btn-primary btn-sm" onclick={handleApply}>应用 YAML</button>
    <button class="btn btn-default btn-sm" onclick={syncFromFilters}>从筛选器同步</button>
  </div>
</div>
...
<style>
  ...
  .yaml-actions { margin-top: 8px; display: flex; gap: 8px; }
  .yaml-error { color: var(--error-text); font-size: 12px; margin-top: 4px; }
</style>
