...
function bumpVersion() { logicVersion.update(n => n + 1); }
...
export function removeLogicItemById(node, id) {
  for (let i = 0; i < node.items.length; i++) {
    ...
    if (item.episode?.logic && removeLogicItemById(item.episode.logic, id)) return true;
  }
  return false;
}
...
export function createEmptyCondition(type) {
  switch (type) {
    ...
    case 'relation': return { relation: { type: '', mode: 'any', conditions: [{ logic: newLogicGroup('and') }] } };
    case 'staff': return { staff: { position: '', mode: 'any', conditions: [{ logic: newLogicGroup('and') }] } };
    case 'episode': return { episode: { mode: 'any', logic: newLogicGroup('and') } };
  ...
  for (let i = 0; i < newItems.length; i++) {
    ...
    if (item.logic) {
      if (item.logic._id === targetId) {
        ...
        const newLogic = { ...item.logic, items: mutator([...item.logic.items]) };
    ...
    if (item.relation?.conditions) {
      ...
      const newConds = item.relation.conditions.map(c => {
        if (c.logic && !changed) {
          if (c.logic._id === targetId) { changed = true; return { logic: { ...c.logic, items: mutator([...c.logic.items]) } }; }
          const updated = updateGroupInTree(c.logic, targetId, mutator);
          if (updated !== c.logic) { changed = true; return { logic: updated }; }
        }
        return c;
      });
      if (changed) { newItems[i] = { relation: { ...item.relation, conditions: newConds } }; return { ...node, items: newItems }; }
    }
    // staff
    if (item.staff?.conditions) {
      let changed = false;
      ...
      if (changed) { newItems[i] = { staff: { ...item.staff, conditions: newConds } }; return { ...node, items: newItems }; }
    }
    // episode
    if (item.episode?.logic) {
      if (item.episode.logic._id === targetId) {
        newItems[i] = { episode: { ...item.episode, logic: { ...item.episode.logic, items: mutator([...item.episode.logic.items]) } } };
        return { ...node, items: newItems };
      }
...
export function addToGroup(group, filter) {
  applyMutation(group._id, items => { items.push(filter); return items; });
}

export function addCondition(group, type, ctx) {
  if (type.startsWith('ep_')) {
    addToGroup(group, { field: { field: type.slice(3), operator: 'contains', value: '' } });
  } else {
    ctx = ctx || group._ctx || 'subject';
    const fc = ctxFieldConfigs(ctx)[type];
    if (fc) {
      const val = fc.type === 'select' ? (fc.options?.[0]?.[0] || '') : '';
      addToGroup(group, { field: { field: type, operator: fc.ops[0], value: val, _special: true } });
    } else {
...
export function removeLogicLeaf(group, idx) {
  applyMutation(group._id, items => { items.splice(idx, 1); return items; });
}

export function addLogicGroupTo(group) {
  const ng = newLogicGroup(group.op === 'and' ? 'or' : 'and');
  applyMutation(group._id, items => { items.push({ logic: ng }); return items; });
    ...
    function cloneAndUpdate(node) {
      ...
      for (let i = 0; i < newItems.length; i++) {
        ...
        if (item.logic) {
          ...
          if (updated !== item.logic) { newItems[i] = { logic: updated }; return { ...node, items: newItems }; }
        }
        if (item.relation?.conditions) {
          let changed = false;
          const newConds = item.relation.conditions.map(c => {
            if (c.logic && !changed) {
              if (c.logic._id === group._id) { changed = true; return { logic: { ...c.logic, op: val } }; }
        ...
        if (item.episode?.logic) {
          if (item.episode.logic._id === group._id) {
            newItems[i] = { episode: { ...item.episode, logic: { ...item.episode.logic, op: val } } };
            return { ...node, items: newItems };
          }
          const updated = cloneAndUpdate(item.episode.logic);
          if (updated !== item.episode.logic) { newItems[i] = { episode: { ...item.episode, logic: updated } }; return { ...node, items: newItems }; }
        }
      }
      return node;
    }
...
export function updateCondition(group, idx, kind, field, value) {
  ...
  applyMutation(group._id, items => { items[idx] = newItem; return items; });
}
...
export const EPISODE_FIELDS = ['name', 'name_cn', 'description', 'airdate', 'duration', 'sort', 'type', 'disc', 'id'];
export const EPISODE_FIELD_LABELS = {
  name: '名称', name_cn: '中文名', description: '简介', airdate: '播出日期',
  duration: '时长', sort: '排序', type: '类型', disc: '碟片', id: 'ID',
};
...
export const PERSON_FIELDS = ['name', 'id', 'type', 'career', 'appear_eps', '简体中文名', '别名', '性别', '生日'];
...
export function opLabel(op) {
  const m = { eq: '=', contains: '包含', not_contains: '不包含', regex: '~=', gt: '>', gte: '>=', lt: '<', lte: '<=', before: '早于', after: '晚于', empty: '为空' };
  return m[op] || op;
}
