...
(function () {
  ...
  function initDB(force = false) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (e) => {
        const db = e.target.result;
        const migrationDone = GM_getValue('personAliasDataVersion', 0);
        if (migrationDone < CONFIG.dbVersion) {
          migrateAliasesToArray(db)
            .then(() => {
              GM_setValue('personAliasDataVersion', CONFIG.dbVersion);
              db.close();
              dbInitialized = true;
              resolve();
            })
  ...
  function migrateAliasesToArray(db) {
    return new Promise((resolve, reject) => {
      ...
      store.openCursor().onsuccess = (e) => {
        ...
        if (cursor) {
          ...
          if (record.personIndex !== undefined && !record.personIndexes) {
            record.personIndexes = [record.personIndex];
            delete record.personIndex;
